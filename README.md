...
backend/            頭脳・ゲームロジック・WebSocketサーバ
  ...
  rag.py            講義/Discord 検索(bge-m3+Qdrant。未投入時はキーワードへ自動フォールバック)
  embeddings.py     bge-m3 埋め込み(build_index と rag で共有)
...
## 講義データを取り込む(Phase 0 / RAG)
店長が「本当に講義を見た感」で答えるための知識源(講義動画 + Discord)を、
オフラインで前処理して **bge-m3 埋め込み + Qdrant** に投入する。投入後は
`backend/rag.py` が**自動で**ベクトル検索に切り替わる(未投入・依存未導入なら
キーワード方式に自動フォールバックするので、Phase 1〜4 はデータ無しでも動く)。

```bash
# 0) 依存(RAG用)。bge-m3 は初回に約2GBのモデルを取得する。
pip install qdrant-client FlagEmbedding yt-dlp

# 1) 講義動画 → タイムスタンプ付き文字起こし(YouTube URL でもローカル動画でも可)
#    GPUサーバなら GACHA_WHISPER_MODEL=large-v3 GACHA_WHISPER_DEVICE=cuda が高精度・高速
python ingest/transcribe.py "https://youtu.be/xxxx" "https://youtu.be/yyyy"
#    → ingest/data/lectures/*.json

# 2) (任意)Discord 履歴。DiscordChatExporter で JSON を吐き ingest/data/discord/ に置く
#    DiscordChatExporter.Cli export -t <TOKEN> -c <CHANNEL_ID> -f Json -o out.json
python ingest/discord_parse.py            # → ingest/data/discord.json

# 3) チャンク分割 + 埋め込み → Qdrant 投入(冪等。何度流しても同じ状態)
python ingest/build_index.py              # --dry-run でチャンク数だけ確認も可

# 4) クイズ下書きを生成(Claude)。★必ず人手検証してから採用する
python ingest/gen_quiz.py --per-lecture 5 # → ingest/quiz_candidates.json
#    良問だけ needs_review/source_text を外して ingest/quiz_bank.json に移す
```

- **Qdrant は既定で「埋め込みモード」**(`ingest/qdrant_db/` にローカル永続、Docker不要)。
  本番/研究室は `GACHA_QDRANT_URL=http://qdrant:6333` を指定すれば Docker/サーバの
  Qdrant を使う(同じコードのまま)。埋め込みモードは path を排他ロックするので、
  **build_index 実行中はバックエンドを止める**こと(URL 指定なら同時アクセス可)。
- `ingest/data/`・`ingest/qdrant_db/`・`quiz_candidates.json` は重い/秘匿なので
  git 管理外(`.gitignore` 済み)。
- クイズは**実行時生成しない**(誤判定防止)。事前生成+人手検証したバンクから出題し、
  正誤は `game.py` がサーバ側で照合する。
...
## Docker でまとめて起動(研究室GPU / 本番向け・推奨)
バックエンド + VOICEVOX + Cloudflare Tunnel を**1コマンド**で。今のPCでも研究室GPUサーバでも同じ手順(要 Docker)。
1. `cp .env.example .env` → `.env` を編集(`ANTHROPIC_API_KEY`、テスト用 `GACHA_WALLET` 等)。
2. 起動:
   - CPU(開発/デモ): `docker compose up --build`
   - GPU(研究室): `docker compose -f docker-compose.yml -f docker-compose.gpu.yml up --build`
3. 公開URLを取得: `docker compose logs cloudflared` に出る `https://xxxx.trycloudflare.com` を、
   Vercel の `NEXT_PUBLIC_WS_URL` に `wss://xxxx.trycloudflare.com/ws` として設定。
- `backend` ↔ `voicevox` はコンテナ内ネットワークで自動接続(`VOICEVOX_URL=http://voicevox:50021`)。
  → **VOICEVOX の WSL↔Windows 問題は起きない**(両方コンテナのため)。
- Whisper モデルは `whisper-cache` ボリュームに保持(毎回ダウンロードしない)。
- 停止: `docker compose down`。
- ※このPCで使うには Docker が必要(Windows は Docker Desktop + WSL 連携)。研究室サーバは通常導入済み。

### GPUサーバで講義データを一括処理(Phase 0 バッチ / large-v3)
研究室サーバは端末で直接ライブラリを入れられないので、**同じイメージを使ったワンショット実行**で
文字起こし〜インデックス構築まで回す。GPU 予約と `large-v3`/cuda/float16 は `docker-compose.gpu.yml`
が付与し、`./ingest` は bind mount でホストに出力が残る(`whisper-cache` に large-v3/bge-m3 を保持)。

```bash
GPU="-f docker-compose.yml -f docker-compose.gpu.yml"
cp .env.example .env      # ANTHROPIC_API_KEY を設定(gen_quiz で使用)

# 0) GPUイメージをビルド(CUDA/cuDNN 同梱)
docker compose $GPU build backend

# 1) 文字起こし(URLをまとめて渡す。--no-deps で voicevox/tunnel は起こさない)
docker compose $GPU run --rm --no-deps backend \
  python ingest/transcribe.py "URL2" "URL3" ... "URL11"   # → ./ingest/data/lectures/*.json

# 2) (任意)Discord: ./ingest/data/discord/ に JSON を置いてから
docker compose $GPU run --rm --no-deps backend python ingest/discord_parse.py

# 3) チャンク分割+bge-m3埋め込み→Qdrant(初回は bge-m3 ~2GB をDL)
docker compose $GPU run --rm --no-deps backend python ingest/build_index.py

# 4) クイズ下書き生成(→ ./ingest/quiz_candidates.json。人手検証して quiz_bank.json へ)
docker compose $GPU run --rm --no-deps backend python ingest/gen_quiz.py --per-lecture 5

# 5) 本番起動(bind mount 済みの ./ingest/qdrant_db を rag.py が自動でベクトル検索に使う)
docker compose $GPU up -d
```

- **注意(埋め込みモードの排他ロック)**: 既定の Qdrant は `./ingest/qdrant_db` をロックするため、
  **build_index 実行中は backend を止めておく**(手順どおり build→up の順なら問題なし)。
  常時同時アクセスしたいなら compose に `qdrant` サービスを足し `GACHA_QDRANT_URL` を向ける。
- 埋め込みも GPU で速くするなら `-e GACHA_EMBED_FP16=1` を run に付ける(小規模ならCPUでも一瞬)。
- 1本目(このPCで small/CPU 実行)と混在してよい。品質を揃えたいなら URL1 も含めて large-v3 で流し直す。
...
- [~] **Phase 0**: データ前処理 — パイプライン実装済み(transcribe/discord_parse/build_index/
      gen_quiz、rag.py を Qdrant 化・自動フォールバック付き、build→検索の往復を検証済み)。
      残り=**実データ投入**(YouTube URL → 文字起こし → 投入)と**クイズ人手検証**。
