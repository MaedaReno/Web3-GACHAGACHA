...
# 取引開始時にサーバが「運」を1回だけ引き、この取引の下限を決める。
#   通常交渉: 約 BASE_FLOOR_PROB の確率で 1500 まで、外れると 1800 止まり。
#   クイズ正解: 下限が一段深く解錠され、通常 1000 / まれに 500(=最安)。
LIST_PRICE = 2000            # 元値(交渉の出発点)
BASE_FLOOR_PRICE = 1500      # 当たり運のときの通常下限(会話が普通でもここまで)
UNLUCKY_FLOOR_PRICE = 1800   # 外れ運のときの下限(1500 までは行けない)
BASE_FLOOR_PROB = 0.7        # 1500 まで下げられる確率(約70%)
ABSOLUTE_MIN_PRICE = 500     # 何をしても絶対に割らない最安値(=クイズ大当たり時のみ)

# --- クイズ報酬(3ターン制では最大1問。任意で挑戦) ---
QUIZ_FLOOR_PRICE = 1000        # クイズ正解時の通常下限
QUIZ_JACKPOT_PRICE = 500       # クイズ正解時にまれに出る大当たり下限(=最安)
QUIZ_JACKPOT_PROB = 0.1        # 大当たり(500)が出る確率(めったに出さない)
MAX_QUIZ_QUESTIONS = 1         # 1取引で出題できる上限

# --- 値切り交渉(ichigo_gatya 参考: ターン制・会話の質で下げる・単調減少) ---
# お客さんの発話1回=1ターン。最終ターンでは店主が必ず取引を締める。
#   1ターン目: 元値を伝えて軽く交渉。
#   2ターン目: 「クイズを解くともっと安くできる」と挑戦するか問いかける。
#   3ターン目(最終): 挑戦するなら1問出して採点→締める。しないならそのまま締める。
